...
async function startBot() {
  ...
  // Check if sessionID is provided and process TroyBot! format session
  if (config.sessionID && config.sessionID.startsWith('TroyBot!')) {
    try {
      const [header, b64data] = config.sessionID.split('!');

      if (header !== 'TroyBot' || !b64data) {
        throw new Error("❌ Invalid session format. Expected 'TroyBot!.....'");
      }
      ...
      console.log('📡 Session : 🔑 Retrieved from TroyBot Session');

    } catch (e) {
      console.error('📡 Session : ❌ Error processing TroyBot session:', e.message);
