...
const APIs = {
  ...
  // Song Download APIs
  // Cobalt - High quality downloader (320kbps audio, 1080p video)
  getCobaltAudioByUrl: async (youtubeUrl) => {
    ...
    if (res?.data?.url && (res.data.status === 'stream' || res.data.status === 'tunnel' || res.data.status === 'redirect')) {
      return { download: res.data.url, title: res.data.filename || '' };
    }
    throw new Error('Cobalt audio returned no download URL');
  },

  getCobaltVideoByUrl: async (youtubeUrl) => {
    const res = await axios.post('https://api.cobalt.tools/', {
      url: youtubeUrl,
      videoQuality: '1080',
      filenameStyle: 'pretty',
    ...
    throw new Error('Cobalt video returned no download URL');
  },
