...
module.exports = {
  ...
  async execute(sock, msg, args) {
    try {
      ...
      // Get video: Cobalt first (1080p), then fallbacks
      let videoData;
      const videoApis = [
        () => APIs.getCobaltVideoByUrl(videoUrl),
      ...
      for (const apiFn of videoApis) {
        try {
          videoData = await apiFn();
          if (videoData && videoData.download) { videoFetched = true; break; }
        } catch (e) { continue; }
      }
      if (!videoFetched) throw new Error('All video APIs failed');
