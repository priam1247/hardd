...
module.exports = {
  ...
  async execute(sock, msg, args) {
    try {
      ...
      // List of API methods to try — Cobalt first for 320kbps quality
      const apiMethods = [
        { name: 'Cobalt', method: () => APIs.getCobaltAudioByUrl(video.url) },
      ...
      // Build proper file name: "Song Title - Artist Name.mp3"
      const songTitle = (audioData.title || video.title || 'Unknown Title').trim();
      const songArtist = (video.author && video.author.name ? video.author.name : (audioData.artist || '')).trim();
      const cleanTitle = songTitle.replace(/[^\w\s\-]/g, '').trim();
      const cleanArtist = songArtist.replace(/[^\w\s\-]/g, '').trim();
      const fileName = cleanArtist
        ? `${cleanTitle} - ${cleanArtist}.${finalExtension}`
        : `${cleanTitle}.${finalExtension}`;
      const audioCaption = cleanArtist
        ? `🎵 *${songTitle}*\n👤 ${songArtist}`
        : `🎵 *${songTitle}*`;

      // Send as document so WhatsApp always shows the exact song + artist filename
      await sock.sendMessage(chatId, {
